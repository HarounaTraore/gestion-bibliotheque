<template>
  <div>
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr v-for="livre in livres" :key="livre.id">
          <td>{{ livre.titre }}</td>
          <td>
            <button class="btn btn-sm m-2" @click="voirDetails(livre)">
    ...
    <button class="btn btn-primary" @click="ajouterLivre = !ajouterLivre">
      Ajouter un livre
    </button>
    ...
    <div v-if="livreSelectionne" class="card mt-3">
      <div class="card-body">
        ...
        <p class="card-text">
          <strong>Titre:</strong> {{ livreSelectionne.titre }}
        </p>
        <p class="card-text">
          <strong>Auteur:</strong> {{ livreSelectionne.auteur }}
        </p>
        <button class="btn btn-secondary" @click="livreSelectionne = null">
          Fermer
        </button>
      </div>
    </div>
